...
def ensure_tags(client, module, resource_arn, existing_tags, tags, purge_tags):
  ...
  if tags_to_remove:
    ...
    try:
      ...
      tags_list = [{'Key': key, 'Value': existing_tags.get(
          key)} for key in tags_to_remove]
...
def pem_chain_split(module, pem):
  ...
  if len(pem_arr) == 0:
    ...
    module.fail_json(
        msg="Unable to split certificate chain. Possibly zero-length chain?")
...
def renew_certificate(client, module, acm, certificate, desired_tags):
  ...
  if cert_type in ['AMAZON_ISSUED', 'PRIVATE']:
    ...
    if cert_status == 'ISSUED':
      ...
      # if datetime.now() > (cert.get('NotAfter') - timedelta(days_before_expiry)):
  ...
  if eligible_for_renewal:
    if module.check_mode:
      module.exit_json(
          changed=True, msg="Would have renewed certificate if not in check mode")
    try:
...
def wait_for_validation_records(client, module, acm, cert_arn):
  ...
  When requesting a public certificate, it may take several seconds for
  the DNS|EMAIL validation records to be generated.
  """
  ...
  while time.time() < deadline:
    ...
    if len(domain_validation_options or []) == 0:
...
def request_certificate(client, module, acm, desired_tags):
  ...
  if sum([(module.params[a] is not None) for a in absent_args]) < 2:
    module.fail_json(
        msg="When requesting a certificate, all of 'name_tag' and 'domain_name' must be specified")
  ...
  if ca_arn is None:
    ...
    if validation_method is None:
      module.fail_json(
          msg="The 'validation_method' parameter must be specified when requesting a public certificate from ACM")
  else:
  ...
  if (
      cert_options is not None
      and
      cert_options.get(
          'certificate_transparency_logging_preference') is not None
  ):
    options['CertificateTransparencyLoggingPreference'] = cert_options.get(
        'certificate_transparency_logging_preference')
  ...
  tags_list = [
      {'Key': key, 'Value': desired_tags.get(key)} for key in desired_tags]
  parameters = {
  ...
  if cert_request.get('subject_alternative_names') is not None:
    parameters['SubjectAlternativeNames'] = cert_request.get(
        'subject_alternative_names')
...
def update_imported_certificate(client, module, acm, old_cert, desired_tags):
  ...
  if ('tags' not in old_cert) or ('Name' not in old_cert['tags']):
    # shouldn't happen
    module.fail_json(
        msg="Internal error, unsure which certificate to update", certificate=old_cert)
  if module.params.get('name_tag') is not None and (old_cert['tags']['Name'] != module.params.get('name_tag')):
    ...
    module.fail_json(
        msg="Internal error, Name tag does not match", certificate=old_cert)
  if 'certificate' not in old_cert:
    # shouldn't happen
    module.fail_json(
        msg="Internal error, unsure what the existing cert in ACM is", certificate=old_cert)
      ...
      certificate_request=dict(
          ...
          options=dict(
              ...
              options=dict(
                  ...
                  default=None,
                  options=dict(
                      certificate_transparency_logging_preference=dict(
    ...
    if desired_tags is not None:
      if 'Name' in desired_tags:
        if desired_tags['Name'] != module.params['name_tag']:
          module.fail_json(
              msg="Value of 'name_tag' conflicts with value of 'tags.Name'")
      else:
